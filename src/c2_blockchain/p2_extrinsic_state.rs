...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        ...
        let mut expecteed_state = self.state;

        let mut is_valid = true;
        for header in chain {
            expecteed_state += header.extrinsic;
            is_valid &= header.parent == expected_parent
                && header.height == expected_height
                && header.state == expecteed_state;
            expected_parent = hash(&header);
            expected_height += 1;
        }
        is_valid
    }
...
fn build_valid_chain(n: u64) -> Vec<Header> {
    let mut header_chain = Vec::with_capacity(n as usize); // Optimize by setting the initial capacity

    let mut header = Header::genesis();
    for _ in 0..n {
        header_chain.push(header.clone());
        // Let extrinic be const for now
        header = header.child(
            hash(
                &SystemTime::now()
...
fn build_an_invalid_chain() -> Vec<Header> {
    ...
    let mut invalid_header = genesis.child(50);
    invalid_header.height = 5;
    let child_of_invalid_header = invalid_header.child(32);

    vec![genesis, child_of_invalid_header, invalid_header]
}
...
fn build_forked_chain() -> (Vec<Header>, Vec<Header>) {
    (build_valid_chain(3), build_valid_chain(3))
